...
import {getTokenAddress,getTokenAddressBalance,increaseAllowance,} from '../utils/context';
...
async function fetchTokenBalance(){
  const bal = await getTokenAddressBalance(name,walletAddress);
  console.log(bal);
...
async function fetchTokenAddress(){
  ...
  <div className="flex mx-2 border-[1px[ rounded-l rounded-r-lg border-[#7765F3]">
    <div className="flex items-center bg-zinc-90 text-zinc-300 w-fit p-2 px-3 rounded-l-lg">
      <p className="text-sm">
        {name}
      </p>
