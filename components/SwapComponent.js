...
const SwapComponent = () => {
  ...
  const isReversed = useRef();

  const INCREASE_VALUE= "Increase allowance";
  ...
  useEffect(()=>{
    ...
    //resetting the isReversde value if its set
  ...
  px-6 rounded-xl">
    ...
    <button className={getSwapBtnClassName()}
      onClick={()=>{
        if(swapBtnText===INCREASE_VALUE) handleIncreaseAllowance();
...
async function handleSwap(){
  if(srcToken === ETH && destToken!==ETH){
    ...
    // Check whether there is allowance when the swap deals with tokenToEth/
    //tokenToToken
    setTxPending(true);
...
function handleReverseExchange(e){
  //Setting the isReverse value to prevent the input/output values
  //being calculated in their respective side - effects
  isReversed.current = true;

  // 1. Swap tokens (srcToken <-> destToken)
  // 2. Swap values (input <-> outputValue)
...
function getSwapBtnClassName(){
  ...
  className += swapBtnText === INCREASE_VALUE ? "bg-yellow-600":"";
  return className;
}
